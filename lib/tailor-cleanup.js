...
function needsCommaSpliceFix(summary) {
  ...
  const beforeComma = match[0].replace(/,\s*$/, "");
  if (/\bwith\b/i.test(beforeComma)) return false;
...
function dedupeAndFilterSkills(skills, sourceResumeText) {
  ...
  const rawLines = expandSkillsToLines(skills)
    .map((line) => line.replace(/^•\s*/, "").trim())
    .filter(Boolean);
  if (!rawLines.length) return skills.trim();

  if (isCategorizedSkillsBlock(rawLines.join("\n"))) {
    /** @type {string[]} */
    const kept = [];

    for (let line of rawLines) {
      line = cleanProseFragment(line);
      if (!line) continue;
...
export function cleanupTailoredResumeLogic(tailored, sourceResumeText, targetRole = "") {
  ...
  const experience = (tailored.experience || []).map((entry) => ({
    ...
    bullets: (entry.bullets || [])
      .map((bullet) => cleanProseFragment(String(bullet ?? "")))
      .filter(Boolean),
  }));
