...
export function parseJsonFromText(text) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const assembled = tryAssemblePartialObject(repairJson(candidate));
...
function collectJsonCandidates(text) {
  ...
  return candidates.sort((a, b) => {
    const scoreDiff = scoreJsonCandidate(b) - scoreJsonCandidate(a);
...
function scoreJsonCandidate(text) {
  ...
  if (text.includes('"atsScore"')) score += 5;
 ...
 * @returns {string | null}
 */
function findPropertyKeyMatch(text, propertyName, { last = false } = {}) {
  const keyPattern = new RegExp(`"${propertyName}"\\s*:`, "gi");
  if (!last) return keyPattern.exec(text);

  let match = null;
  let current = keyPattern.exec(text);
  while (current) {
    match = current;
    current = keyPattern.exec(text);
  }
  return match;
}
 ...
 * @returns {string | null}
 */
function extractScopedObjectSlice(text, scopeProperty) {
  const scopeOptions = { last: true };
  const match = findPropertyKeyMatch(text, scopeProperty, scopeOptions);
...
function extractPropertyValueSlice(text, propertyName, { last = false } = {}) {
  const match = findPropertyKeyMatch(text, propertyName, { last });
  if (!match) return null;
...
/**
 * Prefer resume fields from inside a tailoredResume/resume block so atsScore.summary
 * is never mistaken for the virtual resume summary during partial assembly.
 * @param {string} text
 * @param {string} propertyName
 * @param {{ last?: boolean }} [options]
 * @returns {unknown}
 */
function parseResumePropertyValue(text, propertyName, options = {}) {
  for (const scopeProperty of ["tailoredResume", "tailored_resume", "resume"]) {
    const scopeSlice = extractScopedObjectSlice(text, scopeProperty);
    if (!scopeSlice) continue;

    const scoped = parsePropertyValue(scopeSlice, propertyName, options);
    if (scoped !== undefined) return scoped;
  }

  return parsePropertyValue(text, propertyName, options);
}
 ...
 * @returns {unknown}
 */
function parsePropertyValue(text, propertyName, options = {}) {
  const slice = extractPropertyValueSlice(text, propertyName, options);
...
function tryAssemblePartialObject(text) {
  const resumeOptions = { last: true };
  const tailoredResume =
    parsePropertyValue(text, "tailoredResume", resumeOptions) ??
    parsePropertyValue(text, "tailored_resume", resumeOptions) ??
    parsePropertyValue(text, "resume", resumeOptions);
  ...
  const hasResumeFields =
    parseResumePropertyValue(text, "contact") ||
    parseResumePropertyValue(text, "experience") ||
    parseResumePropertyValue(text, "summary");
  ...
  if (hasResumeObject) {
    ...
    assembled.tailoredResume = {
      contact: parseResumePropertyValue(text, "contact") ?? {},
      summary: parseResumePropertyValue(text, "summary") ?? "",
      experience: parseResumePropertyValue(text, "experience") ?? [],
      education: parseResumePropertyValue(text, "education") ?? [],
      skills: parseResumePropertyValue(text, "skills") ?? "",
      other: parseResumePropertyValue(text, "other") ?? "",
    };
  }
  ...
  const atsOptions = { last: true };
  const atsScore =
    parsePropertyValue(text, "atsScore", atsOptions) ??
    parsePropertyValue(text, "ats_score", atsOptions) ??
    parsePropertyValue(text, "atsscore", atsOptions) ??
    parsePropertyValue(text, "ats", atsOptions);
...
/**
 * Build a tailor response object from individually parsed JSON properties.
 * @param {string} text
 * @returns {Record<string, unknown> | null}
 */
export function assembleTailorJsonFromText(text) {
  return tryAssemblePartialObject(text);
}
 ...
 * @returns {unknown}
 */
export function extractJsonProperty(text, propertyName, options = {}) {
  return parsePropertyValue(text, propertyName, options);
}
