...
import { isCategorizedSkillsBlock } from "./resume-structure.js";
...
function skillListedInField(skillsCsv, skill) {
  ...
  if (!skillsCsv?.trim() || !trimmedSkill) return false;

  if (resumeSupportsKeyword(skillsCsv, trimmedSkill)) return true;

  if (skillsCsv.includes("\n")) {
    for (const line of skillsCsv.split(/\r?\n/)) {
      const trimmedLine = line.trim();
      if (!trimmedLine) continue;
      if (resumeSupportsKeyword(trimmedLine, trimmedSkill)) return true;

      const afterColon = trimmedLine.split(":").slice(1).join(":").trim();
      if (afterColon && resumeSupportsKeyword(afterColon, trimmedSkill)) return true;
    }
  }
  ...
  return parts.some(
    (part) =>
      normalizeTechTerm(part) === normalizeTechTerm(trimmedSkill) ||
      resumeSupportsKeyword(part, trimmedSkill)
  );
}
...
function isEnforceableSkill(skill) {
  ...
  if (
    /^(?:modern|strong|excellent|solid|proven)\s+/i.test(trimmed) &&
    extractTechTermsFromText(trimmed).length === 0
  ) {
    return false;
  }
...
function mergeEnforcedSkills(existingSkills, toPrepend) {
  ...
  if (existing.includes("\n")) {
    ...
    if (isCategorizedSkillsBlock(existing)) {
      const lineCount = existing.split(/\r?\n/).filter((line) => line.trim()).length;
      if (lineCount >= 3) return existing;
