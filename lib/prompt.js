...
const TAILOR_PARTIAL_RESUME_SCHEMA = `{
  "summary": "",
  "skills": "",
  "experience": [
    { "bullets": [""] }
...
const TAILOR_JSON_RESPONSE_SCHEMA = `{
  "tailoredResume": ${TAILOR_PARTIAL_RESUME_SCHEMA},
  "changes": [
...
const TAILOR_RESUME_PROMPT = `# ATS Resume Optimization Prompt

Create a **virtual resume** that is highly optimized for the provided job description.

## Objective

Build a resume that is the strongest possible match for the job description by focusing on:

- Core skills from the job description
- Main responsibilities
- Required technologies
- Key qualifications
- ATS keywords
- Related or adjacent technical skills that logically align with the role

## Optimization Requirements

Analyze the job description and identify:

- Weaknesses to address in the virtual resume
- Missing keywords to include
- Skill gaps to cover
- ATS mismatches to fix
- Underrepresented requirements
- Areas where a strong candidate resume should align with the job

Then build the virtual resume until it becomes a **highly optimized ATS match** for this job.

## Resume Structure

The virtual resume must follow this structure:
...
Language category is always required and always first.
Other categories are optional and may be reordered by importance to the job.
...
Format one category per line: "Languages: Python, JavaScript, Java"

## Experience Bullet Requirements

For each role:

- Include **5–7 bullet points per company**
- Focus on impact, ownership, and measurable outcomes where possible
- Strongly align with job description requirements
- Integrate relevant technologies and keywords naturally
- Ensure all bullets are ATS-friendly and professionally written
- Use friendly simple verbs
...
- Strong alignment with both explicit and implicit job requirements
- Free of irrelevant or generic content
- Truth-safe — do not invent employers, dates, degrees, roles, or metrics`;
...
- skills: multiline string, one category per line, format "Category: skill, skill" (example: "Languages & Front End: JavaScript, TypeScript"). Include every skill and technology from the source.
...
export function buildTailorPrompt({ jobDescription, options = {} }) {
  const { extraInstructions = "", outputFormat = "full resume" } = options;
  const includeCoverLetter = outputFormat === "cover letter + resume";
  const responseSchema = includeCoverLetter
    ? `{
  "tailoredResume": ${TAILOR_PARTIAL_RESUME_SCHEMA},
  "coverLetter": "",
  "changes": [
...
Return only the tailored **summary**, **skills**, and **experience bullets** in tailoredResume.
Each experience entry should contain only a bullets array with 5–7 strings per role.
...
Field notes:
- tailoredResume.summary: summary for the virtual resume
- tailoredResume.skills: multiline string, one category per line (Languages first)
- tailoredResume.experience[].bullets: 5–7 strings per role, no leading bullet symbols
- changes: 3–5 concise adjustment notes
- atsScore.score: integer 0–100; target ${TARGET_ATS_MATCH}+ keyword match with the job description
- atsScore.missingKeywords: only unsupported gaps that cannot be truthfully added
- Plain text only — no markdown bold or ** markers`;
}
