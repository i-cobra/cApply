...
const STOP_WORDS = new Set([
  ...
  "platform", "senior", "junior", "lead", "staff", "principal", "time",
  "workflows", "workflow", "automated", "automation", "reports", "report",
  "tests", "test", "testing", "security", "services", "service", "cloud",
  "assisted", "solutions", "solution", "systems", "system", "applications",
  "application", "development", "developer", "management", "automated",
]);

const MAX_KEYWORDS = 45;
export const TARGET_ATS_MATCH = 90;
 ...
 */
export function computeAtsScoreWithKeywords(resumeText, keywords) {
  if (!keywords.length) {
 ...
 */
export function buildAtsScoreAfterEnrichment(structured, jobDescription, priorAtsScore = null) {
  const resumeText = serializeResume(structured);
  const { score, missing, matched, total } = computeAtsScore(resumeText, jobDescription);

  const localSummary =
    total > 0
      ? `${matched.length} of ${total} job keywords present (${score}% local ATS overlap).`
      : "";
