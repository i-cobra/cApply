...
export async function saveTailorHistory(entries) {
  ...
  try {
    const { syncHistoryToCloud } = await import("./supabase-data.js");
    await syncHistoryToCloud(entries);
  } catch (err) {
...
export async function removeTailorHistoryEntry(id) {
  ...
  await saveTailorHistory(history);
  try {
    const { deleteRemoteJobHistoryEntry } = await import("./supabase-data.js");
    await deleteRemoteJobHistoryEntry(id);
  } catch (err) {
    console.warn("Failed to delete job history from Supabase", err);
  }
  return history;
}
...
export async function clearTailorHistory() {
  ...
  try {
    const { clearRemoteJobHistory } = await import("./supabase-data.js");
    await clearRemoteJobHistory();
  } catch (err) {
    console.warn("Failed to clear job history in Supabase", err);
  }
  return [];
}
 ...
 * @param {string} iso
 */
export function formatRelativeHistoryDate(iso) {
  const date = new Date(iso);
  if (Number.isNaN(date.getTime())) return formatHistoryDate(iso);
...
/**
 * @param {TailorHistoryEntry[]} history
 * @returns {{ total: number, saved: number, applied: number, interview: number, archived: number, avgScore: number | null }}
 */
