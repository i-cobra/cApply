...
import { extractJobKeywords } from "./ats-score.js";
...
function formatTailorKeywordPlan(jobDescription, resume) {
  const jdKeywords = extractJobKeywords(jobDescription).slice(0, 30);
...
- education[].school, degree, dates, details: separate fields
  ...
  const emphasisList = emphasize
    .map((e) => {
      const labels = {
        ...
        skills:
          "reorder skills so JD core technologies appear first; add exact JD terms when the source resume supports them",
        summary:
...
Placement rules:
1. summary: align to the target role title and name 3–5 INCLUDE technologies in the first two sentences.
2. skills: front-load INCLUDE terms using the job posting's exact spelling; add close related terms from the same stack when my background supports them.
3. experience: rewrite recent/relevant bullets to name INCLUDE terms where my work supports them.
...
5. Never add EXCLUDE terms or far-away technologies not supported by my resume.
6. Do not drop important technical terms from my source resume when they are close to the job stack.
${extraInstructions ? `\n## Additional notes\n${extraInstructions}` : ""}
...
- missingKeywords: only 0–5 genuine gaps — JD terms with far meaning distance from my source resume AND not present (or close equivalent) in tailoredResume. Never list a keyword that already appears or has a close technology match in tailoredResume.`;
}
