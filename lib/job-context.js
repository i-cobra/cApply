...
function normalizeJobContextState(state) {
  return {
    ...
    jobPosted: state.jobPosted ?? "",
    jobCreated: state.jobCreated ?? "",
    jobModified: state.jobModified ?? "",
    updatedAt: state.updatedAt ?? new Date().toISOString(),
  };
}

/** @type {Record<string, import("./job-context.js").JobContextState> | null} */
let jobContextsCache = null;

/**
 * @returns {Promise<Record<string, import("./job-context.js").JobContextState>>}
 */
async function loadAllJobContexts() {
  if (jobContextsCache) return jobContextsCache;
...
export function invalidateJobContextsCache() {
  jobContextsCache = null;
}
...
export async function saveJobContext(contextKey, state) {
  ...
  map[contextKey] = normalizeJobContextState({
    ...
    jobPosted: state.jobPosted ?? existing?.jobPosted ?? "",
    jobCreated: state.jobCreated ?? existing?.jobCreated ?? "",
    jobModified: state.jobModified ?? existing?.jobModified ?? "",
