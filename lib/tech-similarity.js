 ...
 * @returns {string[]}
 */
function isSpringCityFalsePositive(text) {
  return (
    /\bSpring,\s*[A-Z]{2}\b/.test(text) &&
    !/\bspring\s+boot\b/i.test(text) &&
    !/\b(?:java|kotlin|hibernate|maven|gradle)\b/i.test(text)
  );
}
...
export function extractTechTermsFromText(text) {
  ...
  const found = new Set();
  const skipSpring = isSpringCityFalsePositive(text);

  for (const term of KNOWN_TERMS) {
    if (term === "spring" && skipSpring) continue;
